...
-- Problema: usuario_missoes não tem automação no banco —
-- todo progresso era recalculado no client.
-- Solução: triggers em leitura_progresso, clube_posts,
-- leitura_conteudo e leitura_citacoes que mantêm usuario_missoes
-- como fonte de verdade.
-- ================================================================

-- 3.1 Adiciona coluna data para dedup diário de missões
--     (substitui o reset via pg_cron — cada dia cria nova linha)
ALTER TABLE usuario_missoes
  ADD COLUMN IF NOT EXISTS data date NOT NULL DEFAULT CURRENT_DATE;

-- Atualiza PK para incluir data
...
-- 3.2 Função central de progresso de missões
...
BEGIN
  ...
  LOOP
    -- Upsert progresso do dia; não incrementa se já concluída
    ...
    -- Marca como concluída e concede XP (apenas uma vez por dia)
...
-- 3.3 Trigger: leitura de páginas → missões de 'paginas_lidas'
...
-- 3.4 Trigger: publicação em clube → missões de 'post_clube'
...
-- 3.5 Trigger: insights (notas e citações) → missões de 'insight'
--     Lookup user_id via leituras.user_id
